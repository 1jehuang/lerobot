...
try:
    ...
    if port_handler_com3.openPort():
        ...
        if port_handler_com3.setBaudRate(1000000):
            ...
            # Try to ping each motor ID from 1 to 6
            for motor_id in range(1, 7):
                print(f"Testing motor ID {motor_id}...")
                comm_result, error = packet_handler.ping(port_handler_com3, motor_id)
                
                if comm_result == scs.COMM_SUCCESS:
                    print(f"  Motor ID {motor_id} responded successfully")
                else:
...
try:
    ...
    if port_handler_com4.openPort():
        ...
        if port_handler_com4.setBaudRate(1000000):
            ...
            for motor_id in range(1, 7):
                print(f"Testing motor ID {motor_id}...")
                comm_result, error = packet_handler.ping(port_handler_com4, motor_id)
