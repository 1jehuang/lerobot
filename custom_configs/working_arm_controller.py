...
class WorkingArmController:
    """Controller for the SO-101 working arm on COM4"""
    ...
    def connect(self):
        ...
        try:
            ...
            self.connected = True
            print(f"Successfully connected to working arm on {self.port}")
            return True
    ...
    def run_interactive_control(self):
        """Run interactive control loop for the arm"""
        if not self.connected:
            if not self.connect():
    ...
    def print_menu(self):
        ...
        print("\n==== SO-101 Working Arm Control Menu ====")
...
if __name__ == "__main__":
    print("=== SO-101 Working Arm Controller ===")
    print("This script provides interactive control of the working arm on COM4")
    print("COM3 was tested and does not respond to commands")
    print("\nWORKING ARM CONTROL INSTRUCTIONS:")
    print("- Select a motor (1-6) using the number keys")
    print("- Move the selected motor using W (positive) and S (negative)")
    print("- Adjust step size with + and - keys")
    print("- Return all motors to rest position with R")
    print("- Toggle torque on/off with T")
    print("- Quit with Q")
    
    controller = WorkingArmController()
